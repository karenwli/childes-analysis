...
if __name__ == '__main__': 
    ...
    def import_english_corpora():
        ...
        child_urls = {"https://childes.talkbank.org/data/Eng-NA/Brown.zip", "https://childes.talkbank.org/data/Eng-NA/Bloom.zip",
                    ...
                    "https://childes.talkbank.org/data/Eng-UK/Thomas.zip", "https://childes.talkbank.org/data/Clinical-MOR/ENNI.zip",
        ...
        for url in child_urls:
            ...
            english_corpora.append(pylangacq.read_chat(url))
    ...
    def import_biling_child():
        ...
        montes_url = "https://childes.talkbank.org/data/Spanish/Montes.zip"
        montes = pylangacq.read_chat(montes_url)
        #1-2yo, pos notated
        ...
        biling_child_corpora = [fuertes, silva, deuchar, reyes, montes]
    ...
    def import_spanish_child():
        ...
        annotated_urls = ["https://childes.talkbank.org/data/Spanish/Marrero.zip", "https://childes.talkbank.org/data/Spanish/Hess.zip",
                         "https://phon.talkbank.org/data/Spanish/Koine.zip","https://childes.talkbank.org/data/Frogs/Spanish-Aguilar.zip",
                         "https://childes.talkbank.org/data/Spanish/FernAguado.zip", "https://childes.talkbank.org/data/Spanish/ColMex.zip",
                         "https://childes.talkbank.org/data/Spanish/OreaPine.zip","https://childes.talkbank.org/data/Spanish/Aguirre.zip",
                         "https://childes.talkbank.org/data/Spanish/GRERLI.zip","https://childes.talkbank.org/data/Spanish/Linaza.zip",
                         "https://childes.talkbank.org/data/Spanish/Ornat.zip","https://childes.talkbank.org/data/Spanish/Remedi.zip",
                         "https://childes.talkbank.org/data/Spanish/Romero.zip", "https://childes.talkbank.org/data/Spanish/SerraSole.zip"
                         ]
        spa_corpora = []
        
        for url in annotated_urls:

            spa_corpora.append(pylangacq.read_chat(url))
        ...
        url_list = ["https://childes.talkbank.org/data/Spanish/BecaCESNo.zip", # 3-11, Child: CHI, Adult: MOT, PAT, AUN, INT, EXP, PAU? (too much overlap)
                    ...
                    "https://childes.talkbank.org/data/Spanish/Shiro.zip", # not annotated
                    "https://childes.talkbank.org/data/Frogs/Spanish-Sebastian.zip", # 3-5, 9, 20, CHI: CHI
                    "https://childes.talkbank.org/data/Spanish/JacksonThal.zip",
        ...
        return [spa_corpora, unannotated_list]
    ...
    #   Sentence endings, reduplication, adjectives as an argument, prepositions, wh words, complementizers, or infinitives
        ...
        eng_stops = ["who", "what", "where", "when", "why", "if", "how", "because"]
    ...
    def good_embedded_verb(token):
        ...
        if token.pos in emb_verb_types:
            good_verb = True
        ...
        for i , token in enumerate(corpus):
            ...
            if i > 0 and i < len(corpus)-4:
                ...
                if token.word == "that" and last_token.pos == "v" and  good_matrix:
                    ...
                    if is_okay_intermediate(last_token.word, [token2]) and token2.pos in emb_verb_types:
                        embedded_conj_verb = True
                    elif is_okay_intermediate(last_token.word, [token2, token3]) and token3.pos in emb_verb_types:
                        embedded_conj_verb = True
                    elif is_okay_intermediate(last_token.word, [token2, token3, token4]) and token4.pos in emb_verb_types:
                        embedded_conj_verb = True
                    elif is_okay_intermediate(last_token.word, [token2, token3, token4, token5]) and token5.pos in emb_verb_types:
                        embedded_conj_verb = True
        ...
        for i, token in enumerate(corpus):
            ...
            if i < len(corpus) - 4:
                ...
                if good_matrix and token.pos == "v":
                    ...
                    if is_okay_intermediate(token.word, [token2]) and token2.pos in emb_verb_types:
                        embedded_conj_verb = True
                    elif is_okay_intermediate(token.word, [token2, token3]) and token3.pos in emb_verb_types:
                        embedded_conj_verb = True
                    elif is_okay_intermediate(token.word, [token2, token3, token4]) and token4.pos in emb_verb_types:
                        embedded_conj_verb = True
                    elif is_okay_intermediate(token.word, [token2, token3, token4, token5]) and token5.pos in emb_verb_types:
                        embedded_conj_verb = True
        ...
        for i, token in enumerate(corpus):
            ...
            if i < len(corpus) - 4 and good_matrix:
                ...
                if embedded_conj_verb == True:
                    ...
                    if root != "decir":
                        try:
                            this_ex = tokenminus3.word + " " + tokenminus2.word + " " + last_token.word + " " + token.word + " (" + root + ") " + token2.word + " (" + token2.pos + ") " + token3.word + " (" + token3.pos  + ") "+ token4.word + " (" +token4.pos +")" + token5.word + " (" +token5.pos +")"
                        #print(tokenminus2.word, last_token.word, token.word + " (" + root + ") " + token2.word + " (" + token2.pos + ") " + token3.word + " (" + token3.pos  + ") "+ token4.word + " (" +token4.pos +")")
                        except TypeError:
                            this_ex = ""
                        null_string = null_string + this_ex + "\n"
    ...
    with open("eng_conj", 'r') as f:
        english_cp_conj = json.load(f)
    ...
    english_corpora = import_english_corpora()
    ...
    #biling_adult = [import_biling_adult()]
    #que_analysis(biling_adult, "biling adult")
    #that_analysis(biling_adult, "biling adult")
    #biling_annotated = annotate_corpus(unannotated_biling, "adult")
    ##biling_ages = adult_split_into_ages(biling_child_corpora)
    #total_biling_child = combine_age_arrays([biling_ages, biling_annotated])
    #que_analysis(total_biling_child, "total biling dir")
    #that_analysis(total_biling_child, "total biling dir")


    # Child data
    #spa_annotated = annotate_corpus(beca)
    #spanish_ages = split_into_ages(spanish_child_corpora)
    #total_spanish_child = combine_age_arrays([spa_annotated, spanish_ages])
    #que_analysis(total_spanish_child, "spanish child")

    #spa_adult_annotated = annotate_corpus(beca, "adult")
    #adult_spanish_ages = adult_split_into_ages(spanish_child_corpora)
    #total_spanish_child_dir = combine_age_arrays([spa_adult_annotated, adult_spanish_ages])
    #que_analysis(total_spanish_child_dir, "spanish child dir")
